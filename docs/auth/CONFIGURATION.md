...
1. **Check Existing Resources:**
   - Review [GitHub Issues](https://github.com/mrnajiboy/ebay-mcp-remote-edition/issues)
...
Or, for MCP clients that support authorization headers, use the MCP
server-issued bearer lookup token shown after OAuth:

```text
X-Ebay-Server-Request: true
Authorization: Bearer <mcp-server-issued-bearer-token>
```

Both server-mode variants resolve the same stored user token record and avoid
requiring a hosted session token for server setups. The MCP server-issued bearer
lookup token is generated by this MCP server on the OAuth callback page. It is
not an eBay user access token, eBay refresh token, eBay client-credentials/app
token, legacy hosted session token, or `ADMIN_API_KEY`; it is only a lookup
credential whose backend TTL is kept aligned with the stored eBay refresh token.
...
**Need Help?** If you encounter issues not covered in this guide, please [open a GitHub issue](https://github.com/mrnajiboy/ebay-mcp-remote-edition/issues) or [start a discussion](https://github.com/mrnajiboy/ebay-mcp-remote-edition/discussions).
